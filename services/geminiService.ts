...
const getMockKundaliAnalysis = (name: string, language: string): {analysis: string, rawData: KundaliData} => {
  ...
  return {
    ...
    rawData: {
      ...
      nakshatra: "Rohini",
      houses: {
        1: ["Ma"], 2: ["Ve"], 3: [], 4: ["Mo"], 5: ["Su", "Me"], 
        6: ["Ra"], 7: [], 8: [], 9: ["Ju"], 10: ["Sa"], 11: [], 12: ["Ke"]
      }
    }
  };
};
...
const getMockChatResponse = (message: string, language: string): string => {
    ...
    const isHindi = language === 'Hindi';
    if (msg.includes('job') || msg.includes('career')) return isHindi ? "करियर में मेहनत का फल मिलेगा।" : "Hard work will pay off in career.";
    if (msg.includes('marriage') || msg.includes('love')) return isHindi ? "रिश्तों के लिए समय अनुकूल है।" : "Favorable time for relationships.";
    if (msg.includes('money')) return isHindi ? "आर्थिक स्थिति सुधरेगी।" : "Financial situation will improve.";
    return isHindi ? "मैं अभी ऑफलाइन मोड में हूँ। कृपया बाद में प्रयास करें।" : "I am currently in offline mode. Please try again later.";
};

// --- Backend API Caller ---
async function callBackendAI(endpoint: string, body: any, fallbackFn: () => any) {
...
export const api = {
    login: async (credentials: any) => {
        try {
            ...
            throw new Error("Login failed. Server offline and user not found locally.");
        }
    },
